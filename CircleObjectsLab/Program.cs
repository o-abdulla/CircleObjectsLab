...
double radius = Validator.GetPositiveInputInt();
...
while (runMethods)
{
    ...
    runMethods = Validator.GetContinue("Would you like the circle to grow?");
    if (runMethods == true)
    {
        shapeCircle.Grow();
    }
    ...
    {
        Console.WriteLine($"Goodbye! The circle radius is {shapeCircle.GetRadius()}");
    }
}
